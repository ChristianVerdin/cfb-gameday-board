...
**Schedule:** four runs a week, each deliberately hours ahead of when it is needed - `0 18 * * 3` (Wed 1 PM CT, builds the coming Thu..Mon slate), `0 12 * * 4` (Thu 7 AM CT), `0 12 * * 5` (Fri 7 AM CT), `0 9 * * 6` (Sat 4 AM CT) and `0 13 * * 6` (Sat 8 AM CT, a backup for the 4 AM run since a dropped run cannot detect itself) - plus `workflow_dispatch` with optional `start`/`end` (YYYYMMDD).
...
**Self-heal:** `scripts/ci_heal.py` runs before `check.py`. If the build dropped kickoff forecasts (a runner-side Open-Meteo wobble, which reads as "Clean outdoor conditions" rather than a visible gap) it rebuilds once and keeps whichever result has fewer failures. A final step re-checks after the push and **fails the job** if the snapshot is still degraded - the board gets the best data either way, and since this project has no Telegram chat id, the failed-run email is the only unattended signal. A red X here means "rebuild locally and push", not "the board is down".
