...
snapshot rebuilds itself four times a week via GitHub Actions (Wed, Thu, Fri
and Sat, each well ahead of kickoff).
...
| File | Role |
| --- | --- |
| `index.html` | UI shell and all CSS. The block between `<!-- games:start -->` and `<!-- games:end -->`, and the week kicker, are generated by `refresh_week.py` - edit the generator, not the block |
